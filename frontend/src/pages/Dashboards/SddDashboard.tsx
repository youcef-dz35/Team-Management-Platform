...
const SddDashboard: React.FC = () => {
    const { data, isLoading, error } = useQuery<SddDashboardData>({
        queryKey: ['sdd-dashboard'],
        queryFn: async () => {
            const response = await api.get('/dashboard/sdd');
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h1 className="text-2xl font-bold text-gray-900">SDD Dashboard</h1>
                <Link
                    to="/reports/new"
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="bg-white rounded-lg shadow p-6">
                    ...
                    <div className="text-3xl font-bold text-blue-600">{stats.projects.active}</div>
                    <div className="text-xs text-gray-400 mt-1">of {stats.projects.total} total</div>
                </div>
                <div className="bg-white rounded-lg shadow p-6">
                ...
                <div className="divide-y divide-gray-200">
                    {stats.recentReports.length === 0 ? (
                        <div className="p-6 text-center text-gray-500">
                            No reports yet. Create your first project report!
                        </div>
                    ) : (
                        stats.recentReports.map((report) => (
                            ...
                            >
                                <div>
                                    <div className="font-medium text-gray-900">{report.project_name}</div>
                                    <div className="text-sm text-gray-500">
                                        {report.period_start} - {report.period_end}
                                    </div>
                                </div>
                                <div className="flex items-center gap-3">
                                    <span className="text-sm text-gray-500">{report.entry_count} entries</span>
                                    <span
